...
public class LecturerUpdateController {
    ...
    @Autowired
    public LecturerUpdateController(LecturerRepository lecturerRepository, PasswordEncoder passwordEncoder) {
    ...
    public ResponseEntity<ApiResponse> createLecturer(@RequestBody LecturerManagementRequest request) {
        try {
            ...
            if (lecturerRepository.existsByUsername(request.getUsername().trim())) {
                return ResponseEntity.badRequest()
                        .body(new ApiResponse(false, "Username đã tồn tại", null));
            }
            if (lecturerRepository.existsByEmail(request.getEmail().trim())) {
                return ResponseEntity.badRequest()
                        .body(new ApiResponse(false, "Email đã tồn tại", null));
            }
            if (lecturerRepository.existsByLecturerCode(request.getLecturerCode().trim())) {
                return ResponseEntity.badRequest()
    ...
    public ResponseEntity<ApiResponse> updateLecturer(@PathVariable Long id, @RequestBody LecturerManagementRequest request) {
        try {
            ...
            if (lecturerRepository.existsByUsernameAndIdNot(request.getUsername().trim(), id)) {
                return ResponseEntity.badRequest()
                        .body(new ApiResponse(false, "Username đã tồn tại", null));
            }

            // Check email exists
            if (lecturerRepository.existsByEmailAndIdNot(request.getEmail().trim(), id)) {
                return ResponseEntity.badRequest()
                        .body(new ApiResponse(false, "Email đã tồn tại", null));
            }

            // Check lecturer code exists
            if (!lecturer.getLecturerCode().equals(request.getLecturerCode().trim()) &&
                lecturerRepository.existsByLecturerCode(request.getLecturerCode().trim())) {
                return ResponseEntity.badRequest()
                        .body(new ApiResponse(false, "Mã giảng viên đã tồn tại", null));
            }
